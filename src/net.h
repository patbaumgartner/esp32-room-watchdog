...
// Starts the services that need an address — the <MDNS_HOSTNAME>.local
// announcement and, for an https Gotify URL, the NTP sync TLS validation
// depends on. Cheap once they are up, so the sensor loop can call it every
// pass; the first successful call blocks for the NTP sync.
void netPoll();
